...
!if ( MPI == 1 ) then
  ...
  if (thermo .eq. 1) then
    ...
    pe=tmp
    call mpi_reduce(QVE,tmp,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierror)
  QVE=tmp
...
!endif!mpi
  ...
  if (mype .eq. 0) then  
     ...
     e1 = time
     e2 = ke
     e3 = pe        ! serves here as scalar energy for temperature
     e4 = vvk       ! kolmogorov wavenumber X energy at this wavenumber
     e5 = v         ! enstrophy
     e6 = vh        ! helicity
     e7 = qve       ! serves here as scalar energy for qv
     e8 = kkol      ! kolmogorov wave#
!     WRITE( 6,5042) AER
     WRITE( 6,5043) 
     WRITE(6, 5044) e1,e2,e3,e4,e5,e6,e7,e8!TIME, KE, PE, VVK, V, vh, QVE, KKOL
     WRITE(81,5044) e1,e2,e3,e4,e5,e6,e7,e8!TIME, KE, PE, VVK, V, vh, QVE, KKOL
...
5042  FORMAT(1X,A90)
5043  FORMAT(7X,'Time',8x,'KE',10x,'PE',10X,'VVK',10x,'V',10x,'H',10x,'S2',6x,'KKOL')
