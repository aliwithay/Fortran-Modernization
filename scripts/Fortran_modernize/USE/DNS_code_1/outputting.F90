...
!     istatus = nf_create(filename,0,idnck(nf))
     istatus = nf_create(filename,OR(NF_CLOBBER,NF_NETCDF4),idnck(nf))
...
!        istatus = nf_create(filenamed,0,idncd(nf))
        istatus = nf_create(filenamed,OR(NF_CLOBBER,NF_NETCDF4),idncd(nf))
!        istatus = nf_create(filenamed,OR(NF_CLOBBER,NF_64BIT_OFFSET),idncd(nf))
        ...
        istatus = nf_def_var(idncd(nf),"R",NF_FLOAT,2,ncdimsd,idr)
        if (istatus .ne. nf_noerr) then 
           print*,'error nf_def_var R'
           call handle_err(istatus)
        endif
        if(thermo .eq. 1)then
           istatus = nf_def_var(idncd(nf),"DR3",NF_FLOAT,2,ncdimsd,iddr3)
           if (istatus .ne. nf_noerr) then 
              print*,'error nf_def_var DR3'
              call handle_err(istatus)
           endif
           istatus = nf_def_var(idncd(nf),"R_CCN",NF_FLOAT,2,ncdimsd,idr_ccn)
           if (istatus .ne. nf_noerr) then
                   print*,'error nf_def_var R_CCN'
                   call handle_err(istatus)
           endif
           istatus = nf_def_var(idncd(nf),"KAPPA",NF_FLOAT,2,ncdimsd,idkappa)
           if (istatus .ne. nf_noerr) then
                   print*,'error nf_def_var KAPPA'
                   call handle_err(istatus)
           endif
...
801	continue
  ...
  if(thermo .eq. 1) then
          ...
          call mpi_bcast(idkappa,1,MPI_INTEGER,0,MPI_COMM_WORLD,istatus)
  endif
...
! dump droplet restart into drop.out.ncf
  ...
  integer :: iproc,nbuf,nsends,nrecvs,ntdump,ndropreal,i,j,nddone
  integer :: istatus,ip,idvar

  real, allocatable, dimension(:) :: xall,yall,zall,rall,dr3all,r_ccnall,kappaall, udropall,vdropall,wdropall,idprealall
...
!endif!mpi
  ...
  allocate( xall(ndroppe*npe), yall(ndroppe*npe), zall(ndroppe*npe), rall(ndroppe*npe))
  allocate( dr3all(ndroppe*npe),r_ccnall(ndroppe*npe),kappaall(ndroppe*npe),udropall(ndroppe*npe), vdropall(ndroppe*npe), wdropall(ndroppe*npe), idprealall(ndroppe*npe) )

!if ( MPI == 1 ) then
  nbuf = ndroppe
  nsends = (npe-1)*(8+thermo*3)
  nrecvs = (8+thermo*3)
!endif!mpi
  junk=udrop
  ncstartd(2) = 1
...
! and second to last element to store nddone (used for collisional growth)
  ...
  if (opt .eq. 0) then 
     ...
     if (mype.eq.0) then
        ...
        istatus = nf_put_vara_real(idncd(ntdump),idr,ncstartd,nccountd,r)
        if (istatus .ne. nf_noerr) then
           print*,'Error in putting r by mype 0'
           call handle_err(istatus)
        endif
        if(thermo.eq.1) then
           istatus = nf_put_vara_real(idncd(ntdump),iddr3,ncstartd,nccountd,dr3)
           if (istatus .ne. nf_noerr) then
              print*,'Error in putting dr3 by mype 0'
              call handle_err(istatus)
           endif
           istatus = nf_put_vara_real(idncd(ntdump),idr_ccn,ncstartd,nccountd,r_ccn)
           if (istatus .ne. nf_noerr) then
              print*,'Error in putting r_ccn by mype 0'
              call handle_err(istatus)
           endif
           istatus = nf_put_vara_real(idncd(ntdump),idkappa,ncstartd,nccountd,kappa)
           if (istatus .ne. nf_noerr) then
              print*,'Error in putting kappa by mype 0'
              call handle_err(istatus)
           endif
        endif
  ...
  do i = 1,npe-1
     ...
     ! send to mype=0
        ...
        call mpi_send(r,nbuf,MPI_REAL,0,idr,MPI_COMM_WORLD,istatus)
        if(thermo .eq. 1) then
                call mpi_send(dr3,nbuf,MPI_REAL,0,iddr3,MPI_COMM_WORLD,istatus)
                call mpi_send(r_ccn,nbuf,MPI_REAL,0,idr_ccn,MPI_COMM_WORLD,istatus)
                call mpi_send(kappa,nbuf,MPI_REAL,0,idkappa,MPI_COMM_WORLD,istatus)
        endif
  ...
  elseif (opt .eq. 1) then
     ...
     call mpi_gather(r,ndroppe,MPI_REAL,rall,ndroppe,MPI_REAL,0,MPI_COMM_WORLD,ierror)
     if(thermo .eq. 1) then
             call mpi_gather(dr3,ndroppe,MPI_REAL,dr3all,ndroppe,MPI_REAL,0,MPI_COMM_WORLD,ierror)
             call mpi_gather(r_ccn,ndroppe,MPI_REAL,r_ccnall,ndroppe,MPI_REAL,0,MPI_COMM_WORLD,ierror)
             call mpi_gather(kappa,ndroppe,MPI_REAL,kappaall,ndroppe,MPI_REAL,0,MPI_COMM_WORLD,ierror)
     endif
     ...
     if (mype .eq. 0) then
        ...
        istatus = nf_put_vara_real(idncd(ntdump),idr,ncstartd,nccountd,rall)
        if (istatus.ne.0) print*,'Error in putting r by mype 0'
        if(thermo.eq.1)then
          istatus = nf_put_vara_real(idncd(ntdump),iddr3,ncstartd,nccountd,dr3all)
          if (istatus.ne.0) print*,'Error in putting dr3 by mype 0'
          istatus = nf_put_vara_real(idncd(ntdump),idr_ccn,ncstartd,nccountd,r_ccnall)
          if (istatus.ne.0) print*,'Error in putting r_ccn by mype 0'
          istatus = nf_put_vara_real(idncd(ntdump),idkappa,ncstartd,nccountd,kappaall)
          if (istatus.ne.0) print*,'Error in putting kappa by mype 0'
        endif
...
!  endif ! mype
  ...
  deallocate( idpreal )
  deallocate( xall,yall,zall,rall,dr3all,r_ccnall,kappaall, udropall,vdropall,wdropall,idprealall )
...
subroutine ncreaddrop(ndropreal,time,nddone) 
  ...
  integer :: idtimesd,ididp,idxd,idyd,idzd,idr,iddr3,idr_ccn,idkappa, idudrop,idvdrop,idwdrop
  ...
  if (mype.eq.0) then
    ...
    if (thermo .eq. 1) then 
     ...
     if(thermo.eq.1) then
        ...
        istatus = nf_inq_varid(idncdd,'KAPPA',idkappa)
        if (istatus .ne. nf_noerr) then
                print*,'Error getting kappa ID'
                call handle_err(istatus)
        endif
     endif
     ...
     do iread=npe-1,0,-1  ! read one block at a time; read first block last
        ...
        istatus = nf_get_vara_real(idncdd,idr,ncstartd,nccountd,r)
        if (istatus .ne. nf_noerr) then
           print*,'Error reading r'
           call handle_err(istatus)
        endif 
        if(thermo .eq. 1)then
          istatus = nf_get_vara_real(idncdd,iddr3,ncstartd,nccountd,dr3)
          if (istatus .ne. nf_noerr) then
             print*,'Error reading dr3'
             call handle_err(istatus)
          endif
          istatus = nf_get_vara_real(idncdd,idr_ccn,ncstartd,nccountd,r_ccn)
          if (istatus .ne. nf_noerr) then
                  print*,'Error reading r_ccn'
                  call handle_err(istatus)
          endif
          istatus = nf_get_vara_real(idncdd,idkappa,ncstartd,nccountd,kappa)
          if (istatus .ne. nf_noerr) then
                  print*,'Error reading kappa'
                  call handle_err(istatus)
          endif
        endif!thermo
...
!        if ( MPI == 1 ) then       
	   ...
	   if(thermo .eq. 1) then
           ...
           call mpi_send(r,nbuf,MPI_REAL,iread,0,MPI_COMM_WORLD,istatus)
           if (istatus.ne.0) print*,'Error sending r to proc', iread
           if(thermo .eq. 1)then
              call mpi_send(dr3,nbuf,MPI_REAL,iread,0,MPI_COMM_WORLD,istatus)
              if (istatus.ne.0) print*,'Error sending dr to proc', iread
              call mpi_send(r_ccn,nbuf,MPI_REAL,iread,0,MPI_COMM_WORLD,istatus)
              if (istatus.ne.0) print*,'Error sending r_ccn to proc', iread
              call mpi_send(kappa,nbuf,MPI_REAL,iread,0,MPI_COMM_WORLD,istatus)
              if (istatus.ne.0) print*,'Error sending kappa to proc', iread
           endif
...
!if ( MPI == 1 ) then
  if (mype.gt.0) then ! get block from proc 0
     ...
     call mpi_recv(r,nbuf,MPI_REAL,0,MPI_ANY_TAG,MPI_COMM_WORLD,status,istatus)
     if (istatus.ne.0) print*,mype,'Error receiving r'
     if(thermo.eq.1)then
        call mpi_recv(dr3,nbuf,MPI_REAL,0,MPI_ANY_TAG,MPI_COMM_WORLD,status,istatus)
        if (istatus.ne.0) print*,mype,'Error receiving dr3'
        call mpi_recv(r_ccn,nbuf,MPI_REAL,0,MPI_ANY_TAG,MPI_COMM_WORLD,status,istatus)
        if (istatus.ne.0) print*,mype,'Error receiving r_ccn'
        call mpi_recv(kappa,nbuf,MPI_REAL,0,MPI_ANY_TAG,MPI_COMM_WORLD,status,istatus)
        if (istatus.ne.0) print*,mype,'Error receiving kappa'
     endif
