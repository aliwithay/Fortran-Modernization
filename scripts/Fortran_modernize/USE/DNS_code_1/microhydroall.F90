...
!	    intent: variable in a function that can be passed in & out. intent(in) can be entered not changed.
	 ...
	 real function rangen(idum)
      ...
      real :: red,dist,dxp,dyp,dzp,tmpv,tmp,rtmp,ltmp
      real(8) :: cql,vtemp,solu,curv,lwctmp
    ...
    ! For double-collision check and collisional growth
      ...
      real :: curtc, r1cube, r2cube, r3cube, frac, xcoll, ycoll, zcoll, pos
      real, allocatable, dimension(:,:) :: nctemp, ngrtemp, ncall
      real, allocatable, dimension(:) :: sorttemp
      real, allocatable, dimension(:) :: rcol, r_ccncol, kappacol, xcol, ycol, zcol, ucol, vcol, wcol
      integer, allocatable, dimension(:) :: idpc, dblcol, icolcomms, icolcommr, icol, idpcol
      real, allocatable, dimension(:) :: xc, yc, yrelc, zc, udropc, vdropc, wdropc, uudropc, vvdropc, wwdropc, rc,r_ccnc, kappac
 ...
 500  format(1x,f16.6,i10,1X,5(e15.8,1x))
...
!! move droplets
      ...
      do id = 1,ndropreal 
         ...
         if(min(ixp(id),izp(id),iyp(id)) .le. 0 .or. max(ixp(id),izp(id)) .gt. N .or. iyp(id) .gt. 2) then
               print*,'droplet location out of boundary'
               print*, ixp(id),iyp(id),izp(id),x(id),y(id)-mype*n2pe*h,z(id), id, r(id)!markd
         endif
       enddo
   if(thermo .eq. 1 .and. gomic .eq. 1) dr3=0.d0
   if(thermo .eq. 1 .and. gomic .eq. 2) then
      ! beginning of macroscopic calculations
      pp=rhoa*Ra*(1.d0+m_w/m_d*qvpp)*temp
      exner = (PP/P0)**RACP
      fv = 1.0
      sumrp = 0.0       ! compute sum of radii**3 /3
      !get droplet reynolds number assumed zero background velocity & compute ventilation effect coefficient
      do id = 1,ndropreal
...
!      sumrp = sum(r(1:ndropreal)*fv(1:ndropreal))
      ...
      call mpi_allreduce(sumrp,tmp,1,mpi_real,mpi_sum,mpi_comm_world,ierror)
      sumrp = tmp
      ...
      !ks =1.d0/(rhow*Rv*temp/(esat*diffvnd1)+rhow*Lat/(Ka1*Temp)*(Lat/(Rv*Temp)-1))
       ...
       do 120 id = 1,ndropreal  
           sumrordeltaq(ixp(id),izp(id),iyp(id)) = sumrordeltaq(ixp(id),izp(id),iyp(id)) + cql*dr3(id)/3.d0
120     continue
     ...
     !! Add averages to scalar fields      
      ...
      do 130 ikx= 1,n1
            ...
            temp = temp0+ttr(ikx,ikz,iky)+LatoverCp*deltaqp
            esat = 2.53d11*exp(-5.42d3/temp)
 ...
 130  continue
      ...
      call mpi_allreduce(rmth,tmp,1,mpi_real,mpi_sum,mpi_comm_world,ierror)
      rmth = tmp
      call mpi_allreduce(rmqv,tmp,1,mpi_real,mpi_sum,mpi_comm_world,ierror)
      rmqv = tmp
    ...
    !!adjust size of droplets & including ventilation coeffient fv
      do id=1,ndropreal
         temp=temp0+ttr(ixp(id),izp(id),iyp(id))+LatoverCp*deltaqp !total temperature at the droplet location
         esat = 2.53d11*exp(-5.42d3/temp)
         curv=2.d0*sigma_sa/(Rv*rhow*temp*r(id))
         if (isolu .eq. 1) then !hygroscopicity parameter-based solute term (see Petters & Kreidenweis 2017 ACP)
            solu = (r(id)**3-r_ccn(id)**3)/(r(id)**3-(1-kappa(id))*r_ccn(id)**3)
         elseif (isolu .eq. 2) then !classical solute express (see Jensen & Nugent 2017 JAS)
            solu = exp(-kappa(id)*r_ccn(id)**3/(r(id)**3-r_ccn(id)**3))
         elseif (isolu .eq. 0) then
            solu = 1.d0
         endif
         diffvnd2=diffvnd1*1.d0/(r(id)/(r(id)+0.104d-6)+diffvnd1/(r(id)*0.036)*sqrt(2.d0*pi/(Ra*temp)))
         ka2=ka1*1.d0/(r(id)/(r(id)+.216d-6)+ka1/(r(id)*.7*rhoa*cp)*sqrt(2.d0*pi/(Ra*temp)))
         ks =1.d0/(rhow*Rv*temp/(esat*diffvnd2)+rhow*Lat/(Ka2*Temp)*(Lat/(Rv*Temp)-1))
         seq = solu * exp(curv) - 1.d0!equillibrium supersat.
         supersatd(id)=s(ixp(id),izp(id),iyp(id))-sp !supersaturation fluctuation
         !if(id .eq. 10 .and. mype .eq. 0) print*,'rad',r(id),'solu=',solu, 'seq=',seq ,'s_env =',sp,'supersatd_fluctuation=',supersatd(id),'supersatd=',s(ixp(id),izp(id),iyp(id))!mark
            vtemp = delt * 3.0d0*r(id)*ks * (s(ixp(id),izp(id),iyp(id))-seq) * fv(id) + r(id)**3 !r_new**3 using r^2dr/dt formula
         if(vtemp .gt. real(isolu)*r_ccn(id)**3 .and. r(id) .lt. rtrunc) then
            dr3(id) = vtemp-r(id)**3
      ...
      if (mype.eq.0 .and. mod(nt-1,micdtout) .eq. 0 )  then
         write(82,196) time,Sp,pp,temp0,thetapp,qvpp,lwc,qvs,rmth,s(ixp(10),izp(10),iyp(10))
      endif !mype
...
160   continue
    ! Possibility to save droplet trajectories
      if (everynd .ge. 1 .and. gomic .gt. 1) then
         if (mod(nt,ndumpdrop) .eq. 0) then
              ...
              ssfols= 0.0
               do id = 1, ndropreal
                 if (mod(idp(id),everynd) .eq. 0) then
                   ...
                   zfols(ifol) = z(id)
                   rfols(ifol) = r(id)
                   ssfols(ifol)= supersatd(id) !supersaturation fluctuation (=s(id)-s_parcel)
                 endif
               enddo ! id
              ...
              call mpi_gather(rfols,sendfol,MPI_REAL,rfolr,sendfol,MPI_REAL,0,MPI_COMM_WORLD,ierror)
              call mpi_gather(ssfols,sendfol,MPI_REAL,ssfolr,sendfol,MPI_REAL,0,MPI_COMM_WORLD,ierror)
               if (mype .eq. 0) then
                 idpfol = 0
                 xfol = 0.0
                 yfol = 0.0
                 ...
                 ssfol = 0.0
                  do i = 1,ndroppe/everynd*npe*5
                   ...
                   if (idpfoli .ne. 0) then
                       ...
                       ssfol(idpfoli/everynd)= ssfolr(i)
                   endif
                  enddo ! i loop
                  do i = 1,ndrop/everynd
                   write(41,500) time, idpfol(i),xfol(i),yfol(i),zfol(i),rfol(i),ssfol(i) 
                  enddo ! i loop
               endif ! mype .eq. 0
...
190	   continue
              ...
              infocols(1:ndropreal,18)= kappa(1:ndropreal)
           endif!npe=m
! exchange droplet infos on the left boundary of each slab with its left neighbours. only do left boundary to avoid repeated collision counts.
           ...
           nbufsendl = colpars*maxinfocol
           nbufrecvr = colparr*maxinfocol
...
! Finally, add droplets from right neighbour to list of potential collision partners.
           ...
           allocate( udropc(ndropcol), vdropc(ndropcol),wdropc(ndropcol), rc(ndropcol))
           allocate(r_ccnc(ndropcol),kappac(ndropcol) )
           ...
           kappac(1:ndropreal) = kappa(1:ndropreal)
...
!
                              if (tc .le. delt ) then
                                 ...
                                 ncdat(nc,24) = kappac(i)
                                 ncdat(nc,25) = kappac(j)
                          endif!tc<delt
...
!           endif
 ...
 220                continue
         ...
         if (colgr == 2 ) then
                 ...
                 icol = 0
                 if (nc > 0) then
                   allocate(rcol(nc), r_ccncol(nc), kappacol(nc),  xcol(nc), ycol(nc), zcol(nc), ucol(nc), vcol(nc), wcol(nc), idpcol(nc))
                   do i = 1,nc
                       if (ncdat(i,21) == 0) then
                           nddone = nddone + 1
                           cols = cols + 1
                           ...
                           kappacol(nddone)=(r1cube*ncdat(i,24)+r2cube*ncdat(i,25))/r3cube !CCN-mass-weighted mixed-kappa after collision
                 ...
                 if (cols > 0) then
                   ...
                   if (nc > 0) then !put back half of the merged droplets and put them at the end
                       ...
                       kappa(ndropreal+1:ndropreal+nddone)=kappacol(1:nddone)
                       ndropreal = ndropreal + nddone
                   endif ! nc > 0
                 endif ! cols > 0
                 if (nc > 0) then
                   deallocate(rcol, r_ccncol, kappacol, xcol, ycol, zcol, ucol, vcol, wcol, idpcol)
                 endif
...
!           endif ! numngr
           ...
           deallocate(ncdat )
           deallocate(ngrdat )
           deallocate(idpc, xc, yc, yrelc, zc, udropc, vdropc, wdropc, rc, r_ccnc, kappac)
      ...
      if (colgr == 2 .or.  (thermo .eq. 1 .and. gomic .ge. 2)) then !dsd output
         if (mod(nt-1,dsdout) .eq. 0) then
            ...
            call mpi_reduce(dsd,dsdtot,nbins,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierror)
		      call mpi_reduce(ccn_log2,ccntot_log2,nbins,MPI_INTEGER,MPI_SUM,0,MPI_COMM_WORLD,ierror)

            if (mype == 0) then
               write(45,145) time, (dsdtot(i), i=1,nbins)
		         write(46,145) time, (ccntot_log2(i), i=1,nbins)
            endif
         endif ! mod
...
!           deallocate( flowu, flowv, floww, dtovertaup, uudrop, vvdrop, wwdrop )
      ...
      allocate( infosendl(nawaymax,10), infosendr(nawaymax,10), inforecvl(nawaymax,10), inforecvr(nawaymax,10) )
...
! transfer of the droplets. Leftward motion
      ...
      if (nrecvr .ge. 1) then
           ...
           kappa(ndropreal+1:ndropreal+nrecvr) = inforecvr(1:nrecvr,10)
      endif
...
! transfor of the droplets. Rightward motion
      ...
      if (nrecvl .ge. 1) then
           ...
           kappa(ndropreal+1:ndropreal+nrecvl) = inforecvl(1:nrecvl,10)
      endif
      ...
      do i = naway,1,-1
           ...
           kappa(nawayid(i))=kappa(ndropreal)
           ndropreal = ndropreal - 1
      enddo
