...
! calculate skewess of u derivative
!  use fred_mod   
  use mpi
  use mpi_mod
  implicit none
...
!  include 'mpif.h'
  ...
  REAL :: TIME,KX,KY,KZ,KXA(IKTX),KYA(IKTY),KZA(IKTZ),v,s,kurt,tmp,wk2
...
! Go to real space
 ...
 15 continue
    ...
    s=tmp
    call mpi_reduce(kurt,tmp,1,MPI_REAL,MPI_SUM,0,MPI_COMM_WORLD,ierror)
    kurt=tmp

  if (mype .eq. 0) then
    v = v/(3.*n1*n2*n3)!variance
    s = -s/(3.*n1*n2*n3)
    s = s/v**1.5
    kurt = kurt/(3.*n1*n2*n3)
    kurt = kurt/v**2
    print*,'Skewness of velocity derivatives = ',s
    print*,'kurtosis of velocity derivatives = ',kurt
  endif
  nxk = cmplx(0.,0.)  
  nyk = cmplx(0.,0.)  
  nzk = cmplx(0.,0.)  
